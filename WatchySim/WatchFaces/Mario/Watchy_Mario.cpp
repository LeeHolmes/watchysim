...
void WatchyMario::drawWatchFace(){
    ...
    //Hour
    display.drawBitmap(X_PADDING, pos == 0 ? Y_PADDING : Y_PADDING + 20, marioNumbers[hour10], NUM_W, NUM_H, GxEPD_BLACK); //first digit
    display.drawBitmap(X_PADDING+NUM_SPACING+NUM_W, pos == 1 ? Y_PADDING : Y_PADDING + 20, marioNumbers[hour01], NUM_W, NUM_H, GxEPD_BLACK); //second digit

    //Minute
    display.drawBitmap(X_PADDING+2*(NUM_SPACING+NUM_W), pos == 2 ? Y_PADDING : Y_PADDING + 20, marioNumbers[minute10], NUM_W, NUM_H, GxEPD_BLACK); //first digit
    display.drawBitmap(X_PADDING+3*(NUM_SPACING+NUM_W), pos == 3 ? Y_PADDING : Y_PADDING + 20, marioNumbers[minute01], NUM_W, NUM_H, GxEPD_BLACK); //second digit   
}